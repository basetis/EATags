...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						log_message('debug', "Set User data to Session");
						$this->ci->session->set_userdata(array(
								'user_id'                => $user->id,
								'username'               => $user->username,
								'status'                 => ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
								'evernote_user_id'       => $user->evernote_user_id,
								'evernote_access_token'  => $user->evernote_access_token,
								'evernote_token_expires' => $user->evernote_token_expires,
						));
	...
	function logout()
	{
		...
		$this->logout_evernote();

		$this->ci->session->sess_destroy();
	}

	function logout_evernote()
	{
		$this->ci->session->set_userdata('evernote_access_token','');
		$this->ci->session->set_userdata('evernote_token_expires','');
		$this->ci->session->set_userdata('evernote_user_id','');
	}
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$language = $this->ci->session->userdata('selected_lang');
	...
	{
		$this->ci->load->model('eat/eat_flickr', 'eat_flickr');
		$this->ci->eat_flickr->flickr_evernote_logout();
		$this->ci->load->model('eat/eat_twitter', 'eat_twitter');
		$this->ci->eat_twitter->twitter_evernote_logout();
		$this->ci->load->model('eat/eat_wordpress', 'eat_wordpress');
		$this->ci->eat_wordpress->wordpress_evernote_logout();
		$this->ci->load->model('eat/eat_gmail', 'eat_gmail');
		$this->ci->eat_gmail->gmail_evernote_logout();
	}
