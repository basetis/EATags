...
class THttpClient extends TTransport {
  ...
  public function flush() {
    ...
    // START OF HACK
    // Sometimes SSL connection fails, give it 5 seconds and try again :3
    $attemps       = 3;
    $sleep_seconds = 5;
    ...
    for ($i=$attemps; $i > 0; $i--) {
      ...
      if ($this->handle_ !== FALSE) {
        ...
        if ($i > 1){
          log_message('error', 'THttpClient: Could not connect to '.$host.$this->uri_);
          log_message('error', 'Trying again...');
          sleep($sleep_seconds);
        }
      }
    }
